    ...
    public static string Compose(string humanText, CompanyPaths paths, string slug, DepartmentDefinition department)
    {
        ...
        return $"""
            ...
            **途中まで書いたファイルを最終名で置かないこと。** こちらは最終名の出現を
            ...
            一時ファイルは**1回で書き上げる**こと（書き足すなら、新しい一時ファイルに全部書き直す）。
            rename にシェルの承認が要るなどでできなければ、一時ファイルのまま turn を終えてよい。
            しばらく書き換わらない一時ファイルは、こちらが最終名にする。

            ### 報告の中身

            次の節をこの順に書くこと。後の工程と人間は、この形を当てにして読む。

            - `## 結果` …… やり終えたか、どこまでできたか
            - `## やったこと`
            - `## 成果物の場所` …… 作った・変えたファイル
            - `## 確かめたこと` …… 走らせたコマンドと終了コード、その結果。走らせていなければ「走らせていない」と書く
            - `## 残っていること`

            長いログは報告に貼らず、`{Path.Combine(directory, "logs")}` に置いて場所だけ書くこと。

            {ReportOutcomes.RequestText}
